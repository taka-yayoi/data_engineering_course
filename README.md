...
3. カタログ/スキーマを設定:
   - **Catalog**: `workspace`
   - **Schema**: 新規作成（例: `sdp_handson_<あなたの名前>`）
4. **空のファイルから開始** を選択
5. 言語は **SQL** を選択
6. **選択** をクリック
...
#### Step 1: ジョブを作成
1. 左メニューから **Workflows** → **Jobs** を選択
2. **Create Job** をクリック
3. タスク設定:
   - **Task name**: `run_pipeline`
   - **Type**: `Pipeline`
   - **Pipeline**: 演習2または演習3のパイプラインを選択
4. ジョブ名を設定（画面上部の「Untitled」をクリック）

#### Step 2: スケジュール設定（オプション）
1. 右側パネルの **Schedules** で **Add schedule**
2. 頻度を設定（例: 毎日 06:00）
3. ⚠️ 演習後は **Pause** にしておく
