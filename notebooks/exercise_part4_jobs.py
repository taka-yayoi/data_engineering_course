...
# MAGIC 
# MAGIC 左メニューから **Workflows** → **Jobs** を選択し、**Create Job** をクリック
# MAGIC 
# MAGIC ### Step 2: タスクの設定
# MAGIC 
# MAGIC | 設定項目 | 値 |
# MAGIC |---------|-----|
# MAGIC | Task name | `run_pipeline` |
# MAGIC | Type | `Pipeline` |
# MAGIC | Pipeline | 演習2または演習3で作成したパイプラインを選択 |
# MAGIC 
# MAGIC ### Step 3: ジョブ名の設定
# MAGIC 
# MAGIC 画面上部の「Untitled」をクリックして、ジョブ名を設定
...
# MAGIC 
# MAGIC 右側パネルの **Schedules** セクションで **Add schedule** をクリック
# MAGIC 
...
# MAGIC | Every | 1 Day |
# MAGIC | At | 06:00 |
# MAGIC | Timezone | Asia/Tokyo |
# MAGIC 
# MAGIC ⚠️ **演習では**: スケジュールは **Pause** しておきましょう（課金を防ぐため）
...
# MAGIC 
# MAGIC ### 手動実行
# MAGIC 1. ジョブ画面で **Run now** をクリック
# MAGIC 
# MAGIC ### 実行結果の確認
# MAGIC 1. **Runs** タブで実行履歴を確認
# MAGIC 2. 各実行の状態:
# MAGIC    - ✅ **Succeeded**: 成功
# MAGIC    - ❌ **Failed**: 失敗
# MAGIC    - 🔄 **Running**: 実行中
